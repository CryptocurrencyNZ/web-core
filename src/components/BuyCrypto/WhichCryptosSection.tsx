...
const WhichCryptosSection: FC = () => {
    const cryptoList = ['Bitcoin', 'Ethereum', 'Monero', 'Dogecoin', 'Shiba Inu', 'Litecoin', 'Binance Coin', 'Cardano', 'Pest Free Token', 'Stablecoins']
                ...
                <div className="flex flex-col md:flex-row gap-10 md:gap-16">
                    ...
                    <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="w-full md:w-1/2">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-zinc-100">Although there are over 20,000 cryptocurrencies in existence, there's decent chance that 95% of them will not be in active development and usage in 15 years time. The majority of the cryptocurrencies in existence are clones of a handful of other coins, with a few differences in code added in. Especially coins designed to pump & dump - otherwise called 'Shitcoins'.</p>
                        </div>

                        <div className="mt-8">
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                                {cryptoList.map((crypto, index) => (
                                    <a key={index} href={`/learn/${crypto.toLowerCase().replace(/\s+/g, '-')}`} className="bg-gradient-to-b from-zinc-800/70 to-zinc-800/50 p-3 rounded-lg border border-zinc-700/40 hover:border-green-500/30 transition-all duration-300 text-center group">
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.2 }} className="w-full md:w-1/2">
                        ...
                        <div className="space-y-4">
                            <p className="text-zinc-100">As of 2023, the IRD estimates 6-10% of New Zealand already own Bitcoin or other digital assets; approximately 300,000 or more Kiwis.</p>

                            <p className="text-zinc-100">At this point almost everyone has that Bitcoiner mate, and we estimate this number will significantly increase in the coming decades as more Kiwis recognize the benefits of global crypto adoption, crypto technological development, price increases, increased awareness and raw utility.</p>

                            <p className="text-zinc-100">Today, most New Zealanders are drawn to buying, trading, using or investing in Bitcoin / crypto for the following set of reasons:</p>
                        </div>

                        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="mt-6 space-y-3">
                            {reasonsToBuy.map((reason, index) => (
                                <motion.div key={index} variants={itemVariants} className="bg-gradient-to-b from-zinc-800/70 to-zinc-800/50 p-4 rounded-lg border border-zinc-700/40">
