...
function shootRandomLaser() {
  ...
  const [x1, y1] = projection(from.coords);
  const [x2, y2] = projection(to.coords);
  ...
  setTimeout(() => {
    ...
    fetch('https://raw.githubusercontent.com/glynnbird/countriesgeojson/refs/heads/master/new%20zealand.geojson')
      ...
      .then(data => {
        ...
        const isInside = (point) => {
          return data.geometry.coordinates.some(polygonCoords => {
            const feature = {
              type: "Feature",
              properties: {},
              geometry: {
    ...
    for (let j = 0; j < cols; j++) {
        ...
        const point = projection.invert([x + pixelSize/2, y + pixelSize/2]);
